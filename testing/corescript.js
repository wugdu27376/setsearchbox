...
function showSearchSuggestions(suggestions) {
    ...
    var showHistoryInSuggest = localStorage.getItem('searchHistoryInSuggestChecked') === 'true';
    var searchHistoryChecked = document.getElementById('searchHistoryCheckbox').checked;
    ...
    if (isInputEmpty && showHistoryInSuggest && searchHistoryChecked) {
        ...
        if (displayHistory.length > 0) {
            ...
            clearDiv.style.padding = isDesktop() ? '6px 10px' : '5px 10px';
            ...
            clearDiv.style.fontSize = isDesktop() ? '11px' : '13px';
            ...
            clearDiv.onmouseover = function() {
                this.style.backgroundColor = '#e0e0e0';
            };
            clearDiv.onmouseout = function() {
            ...
            for (var i = 0; i < displayHistory.length; i++) {
                ...
                tempSpan.style.position = 'absolute';
                tempSpan.style.whiteSpace = 'nowrap';
                tempSpan.style.font = getComputedStyle(div).font;
                tempSpan.textContent = historyText;
                document.body.appendChild(tempSpan);
                
                if (tempSpan.offsetWidth > 200) {
                    while (tempSpan.offsetWidth > 200 && historyText.length > 3) {
                ...
                div.setAttribute('data-fulltext', displayHistory[i].text);
                ...
                (function(fullText) {
                    div.onclick = function() {
                        var urlInput = document.getElementById('urlInput');
                        var autoFillAndJump = document.getElementById('autoFillAndJumpCheckbox') && 
                                              document.getElementById('autoFillAndJumpCheckbox').checked;
                        
                        if (autoFillAndJump) {
                            // === 勾选 autoFillAndJumpCheckbox 时：直接跳转搜索/跳转 ===
                            ...
                            setTimeout(function() {
                                if (document.getElementById('searchSuggestionsCheckbox').checked) {
                                    var currentQuery = urlInput.value.trim();
                                    if (currentQuery !== '') {
    ...
    if (isUrlPattern && inputText.length > 0 && document.getElementById('showVisitWebsiteCheckbox').checked) {
        ...
        // 在 showSearchSuggestions 函数中，找到访问建议的 onclick 事件
// 修改为：
...
visitDiv.onclick = function() {
    ...
    if (autoFillAndJump) {
        // === 勾选 autoFillAndJumpCheckbox 时：直接跳转 ===
        if (engine === 'iFrameFree') {
...
function fetchSearchSuggestions(query) {
    ...
    // 如果查询为空且启用了历史记录在建议中显示，直接调用 showSearchSuggestions
    if (!query || query.trim() === '') {
        var showHistoryInSuggest = localStorage.getItem('searchHistoryInSuggestChecked') === 'true';
        var searchHistoryChecked = document.getElementById('searchHistoryCheckbox').checked;
